...
const masteryLevel: Record<string, number> = { Aware: 1, Understand: 2, Explain: 3, Apply: 4, Teach: 5 };
...
export default async function LearningDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try { item = await getLearning(id); } catch (error) { if (error instanceof EntityNotFoundError) notFound(); throw error; }
  return <article className="grid gap-8">
    ...
    {item.knowledgeLinks.length ? <section className="grid gap-5"><h2 className="type-heading-3">Linked Knowledge</h2>{item.knowledgeLinks.map((link) => <div className="grid gap-2" key={String(link.knowledgeId)}><a className="type-control text-accent" href={`/knowledge/${link.knowledgeId}`}>{(link.knowledge as {title?:string})?.title ?? "Knowledge"}</a><p aria-label={`Mastery level ${masteryLevel[String(link.masteryAfter)]} of 5: ${link.masteryAfter}`} className="type-body-sm m-0 text-muted">Mastery: Level {masteryLevel[String(link.masteryBefore)]} of 5 · {String(link.masteryBefore)} → Level {masteryLevel[String(link.masteryAfter)]} of 5 · {String(link.masteryAfter)}</p></div>)}</section> : null}
    {item.tags.length || item.attachments.length ? <section className="grid gap-3"><h2 className="type-heading-3">Tags & attachments</h2><div className="flex flex-wrap gap-2">{item.tags.map((tag) => <Badge key={(tag as {id:string}).id}>{(tag as {name:string}).name}</Badge>)}</div>{item.attachments.map((attachment) => <p className="type-body-sm m-0 text-muted" key={(attachment as {id:string}).id}>{(attachment as {original_filename:string}).original_filename}</p>)}</section> : null}
